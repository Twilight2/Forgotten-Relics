...
public class EntitySoulEnergy extends EntityThrowable implements IEntityAdditionalSpawnData
{
    ...
    public EntitySoulEnergy(final World par1World, final EntityLivingBase par2EntityLiving, final EntityLivingBase t, final boolean r) {
	...
	public void onUpdate() {
		...
		for(int i = 0; i < steps; i++) {
			...
			if(worldObj.rand.nextInt(steps) <= 1)
				Botania.proxy.sparkleFX(worldObj, particlePos.x + (Math.random() - 0.5) * 0.4, particlePos.y + (Math.random() - 0.5) * 0.4, particlePos.z + (Math.random() - 0.5) * 0.4, rc, gc, bc, 0.8F, 2);
		...
		if (true) {
			...
			if(targetList.contains(target)) {
				...
				this.worldObj.playSoundEffect(target.posX, target.posY, target.posZ, "random.fizz", 0.6F, 0.8F + (float) Math.random() * 0.2F);
