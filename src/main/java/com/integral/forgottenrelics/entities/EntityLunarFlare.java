...
public class EntityLunarFlare extends EntityThrowable implements IEntityAdditionalSpawnData
{
    ...
    protected void onImpact(final MovingObjectPosition mop) {
    	...
    	if (mop.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK) {
    		if (mop.blockX == this.lockX & mop.blockY == this.lockY & mop.blockZ == this.lockZ) {
    			List <EntityLivingBase> affectedEntities = worldObj.getEntitiesWithinAABB(EntityLivingBase.class, this.boundingBox.expand(2.5, 2.5, 2.5));
        		if (affectedEntities.contains(this.getThrower()))
        			affectedEntities.remove(this.getThrower());
        		
        		if (affectedEntities.contains(mop.entityHit))
        			affectedEntities.remove(mop.entityHit);
        		...
        		Main.packetInstance.sendToAllAround(new LunarFlaresParticleMessage(this.lockX+0.5, this.lockY+1.25, this.lockZ+0.5, 48), new TargetPoint(dimension, this.posX, this.posY, this.posZ, 128.0D));
        		
        		worldObj.playAuxSFX(2001, this.lockX, this.lockY, this.lockZ, Block.getIdFromBlock(worldObj.getBlock(this.lockX, this.lockY, this.lockZ)) + (worldObj.getBlockMetadata(this.lockX, this.lockY, this.lockZ) << 12));
        		worldObj.playSoundEffect(this.lockX, this.lockY, this.lockZ, "ForgottenRelics:sound.lunarFlare", 16.0F, 0.8F + (float) Math.random() * 0.2F);
        		
        		if (!this.worldObj.isRemote) 
        		Main.packetInstance.sendToAllAround(new LunarBurstMessage(this.lockX+0.5, this.lockY+1.5, this.lockZ+0.5, 2.0F), new TargetPoint(dimension, this.posX, this.posY, this.posZ, 128.0D));
	...
	public void onUpdate() {
		...
		for(int i = 0; i < steps; i++) {
			Botania.proxy.sparkleFX(worldObj, particlePos.x + (Math.random() - 0.5) * 0.2, particlePos.y + (Math.random() - 0.5) * 0.2, particlePos.z + (Math.random() - 0.5) * 0.2, rc, (float) (0.8F + (Math.random() * 0.2F)), (float) (0.4F + (Math.random() * 0.6F)), 2.0F, 1);
			if(worldObj.rand.nextInt(steps) <= 1)
				Botania.proxy.sparkleFX(worldObj, particlePos.x + (Math.random() - 0.5) * 1.0, particlePos.y + (Math.random() - 0.5) * 1.0, particlePos.z + (Math.random() - 0.5) * 1.0, rc, (float) (0.8F + (Math.random() * 0.2F)), (float) (0.4F + (Math.random() * 0.6F)), 2.4F, 4);
