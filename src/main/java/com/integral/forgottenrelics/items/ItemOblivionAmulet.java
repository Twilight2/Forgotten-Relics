...
public class ItemOblivionAmulet extends ItemBaubleBase implements IBauble, IWarpingGear {
 ...
 public void addInformation(ItemStack par1ItemStack, EntityPlayer par2EntityPlayer, List par3List, boolean par4)
 {
	 ...
	 if (par1ItemStack.hasTagCompound()) {
		 ...
		 par3List.add(StatCollector.translateToLocal("item.ItemOblivionAmuletDamage.lore") + " " + Math.round(par1ItemStack.getTagCompound().getFloat("IDamageStored") * 100.0) / 100.0);
		 par3List.add(StatCollector.translateToLocal("item.FREmpty.lore"));
	}
 }
...
public void onWornTick(ItemStack itemstack, EntityLivingBase entity) {
	...
	if (!entity.worldObj.isRemote & itemstack.hasTagCompound() & Math.random() <= 0.0008) {
		...
		entity.addPotionEffect(new PotionEffect(PotionID, (int) (100 + Math.random()*2000), (int) (Math.random()*3), true));
