		...
		if (Math.random() <= 0.025) {
			...
			if (players.size() > 0) {
				...
				if (randomPlayer.dimension != entity.dimension) {
					...
					((EntityPlayerMP)randomPlayer).mcServer.getConfigurationManager().transferPlayerToDimension((EntityPlayerMP) randomPlayer, dim1, ((WorldServer) randomPlayer.worldObj).getDefaultTeleporter());
					((EntityPlayerMP)entity).mcServer.getConfigurationManager().transferPlayerToDimension((EntityPlayerMP) entity, dim2, ((WorldServer) entity.worldObj).getDefaultTeleporter());
				}
				
				entity.setPositionAndUpdate(pos2.x, pos2.y, pos2.z);
				randomPlayer.setPositionAndUpdate(pos1.x, pos1.y, pos1.z);
				entity.worldObj.playSoundEffect(pos2.x, pos2.y, pos2.z, "mob.endermen.portal", 1.0F, (float) (0.8F + (Math.random() * 0.2)));
				randomPlayer.worldObj.playSoundEffect(pos1.x, pos1.y, pos1.z, "mob.endermen.portal", 1.0F, (float) (0.8F + (Math.random() * 0.2)));
			}
		}
