...
/**
 * This code is heavily based on such of Rod of Unstable Reservoir from Botania,
 * and is generally a direct upgrade of it. 
 * I assume that such an unusual weapon mechanic should take practically
 * useful form, and since original item definitely were not powerful enough
 * to make things happen, I had a honor of creating this relic.
 * @author Integral
 */
...
public class ItemMissileTome extends Item implements IWarpingGear {
	...
	public ItemMissileTome() {
	 ...
	 public void addInformation(ItemStack par1ItemStack, EntityPlayer par2EntityPlayer, List par3List, boolean par4)
	 {
		 if(GuiScreen.isShiftKeyDown()){
			 ...
			 par3List.add(StatCollector.translateToLocal("item.FREmpty.lore")); 
			 par3List.add(StatCollector.translateToLocal("item.ItemMissileTome7_1.lore") + " " + (int) RelicsConfigHandler.nuclearFuryDamageMIN + "-" + (int) RelicsConfigHandler.nuclearFuryDamageMAX + " " + StatCollector.translateToLocal("item.ItemMissileTome7_2.lore"));
	...
	public void onUsingTick(ItemStack stack, EntityPlayer player, int count) {
		if(count != getMaxItemUseDuration(stack) && count % 2 == 0 && !player.worldObj.isRemote) {
			...
			spawnMissile(player.worldObj, player, player.posX + ((Math.random() - 0.5) * 3.1), player.posY + 3.8 + (Math.random() - 0.5 * 3.1), player.posZ + ((Math.random() - 0.5) * 3.1));
