...
public class ItemArcanum extends ItemBaubleBase implements IBauble, IVisDiscountGear {
 ...
 {
	 par3List.add(StatCollector.translateToLocal("item.ItemArcanum1.lore") + " " + this.getVisDiscount(par1ItemStack, par2EntityPlayer, Aspect.ENTROPY) + "%");
...
public void onWornTick(ItemStack itemstack, EntityLivingBase entity) {
	...
	if (entity instanceof EntityPlayer & !entity.worldObj.isRemote & Math.random() <= (0.025*RelicsConfigHandler.arcanumGenRate)) {
	...
	((ItemWandCasting)randomWand.getItem()).addVis(randomWand, randomAspect, 1, true);
		...
		ItemNBTHelper.setInt(replacedStack, "ILifetime", (int) ((1200 + (Math.random() * 6000)) * RelicsConfigHandler.dormantArcanumVisMult));
