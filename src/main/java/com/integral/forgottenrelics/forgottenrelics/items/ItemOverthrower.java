	...
	public boolean overthrow(EntityLivingBase entity, EntityPlayer overthrower) {
		...
		if (y != 124) {
			...
			if (!(entity instanceof EntityPlayer)) {
				...
				MinecraftServer.getServer().getConfigurationManager().transferPlayerToDimension((EntityPlayerMP) entity, -1);
	...
	public void onUsingTick(ItemStack stack, EntityPlayer player, int count) {
		...
		if (!target.isDead) {
			 ...
			 if (count == 1) {
				 ...
				 if (theTarget.dimension != -1 & !theTarget.worldObj.isRemote) {
					...
					if (!gotEffect & !(theTarget instanceof EntityPlayer)) {
