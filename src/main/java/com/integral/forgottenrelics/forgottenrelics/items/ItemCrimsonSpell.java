...
public class ItemCrimsonSpell extends Item implements IWarpingGear {
	...
	public void onUpdate(ItemStack itemstack, World world, Entity entity, int i, boolean b) {/*
 		if (!world.isRemote & entity.ticksExisted % 16 == 0) {
	    	
 			if (ItemNBTHelper.getInt(itemstack, "ICooldown", 0) > 0) {
 				ItemNBTHelper.setInt(itemstack, "ICooldown", (ItemNBTHelper.getInt(itemstack, "ICooldown", 0) - 16));
 			}
 			
 			if (ItemNBTHelper.getInt(itemstack, "ICooldown", 0) % 8 != 0 || ItemNBTHelper.getInt(itemstack, "ICooldown", 0) < 0) {
 				ItemNBTHelper.setInt(itemstack, "ICooldown", 0);
 			}
	 ...
	 public void addInformation(ItemStack par1ItemStack, EntityPlayer par2EntityPlayer, List par3List, boolean par4)
	 {
		 if(GuiScreen.isShiftKeyDown()) {
			 ...
			 par3List.add(StatCollector.translateToLocal("item.ItemCrimsonSpell6_1.lore") + " " + (int) RelicsConfigHandler.crimsonSpellDamageMIN + "-" + (int) RelicsConfigHandler.crimsonSpellDamageMAX + " " + StatCollector.translateToLocal("item.ItemCrimsonSpell6_2.lore"));
	...
	public ItemStack onItemRightClick(ItemStack stack, World world, EntityPlayer player) {
		...
		if (!SuperpositionHandler.isOnCoodown(player)) {
			...
			while(entities.size() == 0 && distance < gRange) {
				...
				vec.y += 0.5;
				entities = player.worldObj.getEntitiesWithinAABB(EntityLivingBase.class, AxisAlignedBB.getBoundingBox(vec.x - (SEARCH_RANGE + superposition), vec.y - (SEARCH_RANGE + superposition), vec.z - (SEARCH_RANGE + superposition), vec.x + (SEARCH_RANGE + superposition), vec.y + (SEARCH_RANGE + superposition), vec.z + (SEARCH_RANGE + superposition)));
			...
			if (entities.size() == 0) {
				notFound = true;
				entities = player.worldObj.getEntitiesWithinAABB(EntityLivingBase.class, AxisAlignedBB.getBoundingBox(player.posX - gRange, player.posY - gRange, player.posZ - gRange, player.posX + gRange, player.posY + gRange, player.posZ + gRange));
			}
			...
			if (WandManager.consumeVisFromInventory(player, new AspectList().add(Aspect.FIRE, this.IgnisCost).add(Aspect.ENTROPY, this.PerditioCost))) {
