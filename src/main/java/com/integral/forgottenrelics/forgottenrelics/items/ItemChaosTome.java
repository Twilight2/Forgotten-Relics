...
public class ItemChaosTome extends Item implements IWarpingGear {
 ...
 public void addInformation(ItemStack par1ItemStack, EntityPlayer par2EntityPlayer, List par3List, boolean par4)
 {
	 if(GuiScreen.isShiftKeyDown()){
		 ...
		 par3List.add(StatCollector.translateToLocal("item.FREmpty.lore")); 
		 par3List.add(StatCollector.translateToLocal("item.ItemChaosTome5_1.lore") + " " + "1-" + (int) RelicsConfigHandler.chaosTomeDamageCap + " " + StatCollector.translateToLocal("item.ItemChaosTome5_2.lore")); 
		 ...
		 par3List.add(" " + StatCollector.translateToLocal("item.FRAerCost.lore") + this.round((((int)this.AerCost*Math.random())/100.0D)*10.0D, 2));
		 par3List.add(" " + StatCollector.translateToLocal("item.FRTerraCost.lore") + this.round((((int)this.TerraCost*Math.random())/100.0D)*10.0D, 2));
		 par3List.add(" " + StatCollector.translateToLocal("item.FRIgnisCost.lore") + this.round((((int)this.IgnisCost*Math.random())/100.0D)*10.0D, 2));
		 par3List.add(" " + StatCollector.translateToLocal("item.FRAquaCost.lore") + this.round((((int)this.AquaCost*Math.random())/100.0D)*10.0D, 2));
		 par3List.add(" " + StatCollector.translateToLocal("item.FROrdoCost.lore") + this.round((((int)this.OrdoCost*Math.random())/100.0D)*10.0D, 2));
		 par3List.add(" " + StatCollector.translateToLocal("item.FRPerditioCost.lore") + this.round((((int)this.PerditioCost*Math.random())/100.0D)*10.0D, 2));
	 }
	 else {
 ...
 public boolean spawnOrb(World world, EntityPlayer player) {
		...
		if (!world.isRemote) {
			...
			orb = new EntityChaoticOrb(world, player, true);
			else 
			orb = new EntityChaoticOrb(world, player, false);
			
			orb.setPosition(originalPos.x + ((Math.random()-0.5D)*3.0D), originalPos.y + ((Math.random()-0.5D)*1.0D), originalPos.z + ((Math.random()-0.5D)*3.0D));
			
			Vector3 motion = (new Vector3(orb.posX, orb.posY, orb.posZ).sub(originalPos.copy())).multiply(0.2D + (Math.random()*0.2D));
 ...
 public void onUsingTick(ItemStack stack, EntityPlayer player, int count) {
	 ...
	 if (count != stack.getMaxItemUseDuration() & count % 2 == 0 & !player.worldObj.isRemote) {
		 ...
		 if (WandManager.consumeVisFromInventory(player, new AspectList().add(Aspect.AIR, (int) (this.AerCost*Math.random())).add(Aspect.EARTH, (int) (this.TerraCost*Math.random())).add(Aspect.WATER, (int) (this.AquaCost*Math.random())).add(Aspect.FIRE, (int) (this.IgnisCost*Math.random())).add(Aspect.ORDER, (int) (this.OrdoCost*Math.random())).add(Aspect.ENTROPY, (int) (this.PerditioCost*Math.random())))) {
