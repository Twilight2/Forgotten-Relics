...
public class ItemTelekinesisTomeLegacy extends Item implements IWarpingGear {
		...
		if(ticksCooldown == 0) {
			...
			if(item == null) {
				...
				while(entities.size() == 0 && distance < 32) {
					...
					if (entities.size() > 0)
					for (int counter = 0; counter <= entities.size()-1; counter++) {
							Entity itemS = entities.get(counter);
							if (itemS instanceof EntityXPOrb || itemS instanceof EntityAspectOrb || itemS instanceof EntityThaumcraftBoss || itemS instanceof EntityDoppleganger) {
			...
			if(item != null) {
				...
				if(WandManager.consumeVisFromInventory(player, new AspectList().add(Aspect.AIR, 6).add(Aspect.ORDER, 6))) {
					if(item instanceof EntityItem)
					...
					for(int i = 0; i < 4; i++) {
						...
						Botania.proxy.wispFX(world, item.posX + item.width / 2, item.posY + item.height / 2, item.posZ + item.width / 2, r, g, b, s, xm, ym, zm);
					}
					
					for (int counter = 0; counter <= 8; counter ++)
						Main.proxy.spawnSuperParticle(world, "portalstuff", item.posX, item.posY, item.posZ, (Math.random() - 0.5D)*3.0D, (Math.random() - 0.5D)*3.0D, (Math.random() - 0.5D)*3.0D, 1.0F, 64);
	...
	public static void lightningAttack(EntityPlayer player, Entity target, ItemStack stack, World world) {
		...
		if (WandManager.consumeVisFromInventory(player, new AspectList().add(Aspect.AIR, 80).add(Aspect.ORDER, 50).add(Aspect.FIRE, 200))) {
			
			for (int counter = 0; counter <= 3; counter++)
			Main.proxy.lightning(player.worldObj, player.posX, player.posY+1.0, player.posZ, TVec.x, TVec.y, TVec.z, 20, (float) ((1/player.getDistanceToEntity(target)) * (Math.random()) * 4.0F), (int) (player.getDistanceToEntity(target)*1.2F), 0, 0.175F);
			player.worldObj.playSoundAtEntity(player, "thaumcraft:zap", 1F, 0.8F);
			
			target.attackEntityFrom(new DamageRegistryHandler.DamageSourceTLightning(player), (float) (16 + (24 * Math.random())));
			
			}
			
			if (player.isSneaking())
			if (WandManager.consumeVisFromInventory(player, new AspectList().add(Aspect.AIR, 150).add(Aspect.ORDER, 80)) & !player.worldObj.isRemote) {
